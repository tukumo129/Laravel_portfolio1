					...
					<form method="post" action="{{ route('update_holiday_setting') }}">
						...
						<table class="table table-borderd">
							...
							<tr>
								<th>祝日</th>
								<td>
									<input type="radio" name="flag_holiday" value="{{ $FLAG_OPEN }}" {{ ($setting->isOpenHoliday()) ? 'checked' : '' }} id="flag_holiday_open" />
									<label for="flag_holiday_open">営業日</label>
									<input type="radio" name="flag_holiday" value="{{ $FLAG_CLOSE }}" {{ ($setting->isCloseHoliday()) ? 'checked' : '' }} id="flag_holiday_close" />
